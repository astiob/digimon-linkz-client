...
public class CMD_PvPTop : CMD
{
	...
	private void SetTimeSchedule()
	{
		...
		while (num < array.Length && num < this.startTimeLabelArray.Length && num < this.endTimeLabelArray.Length)
		{
			...
			{
				dateTime3 = new DateTime?(TimeUtility.DateTimeDifferenceServer(DateTime.Parse(array[num + 1].startHour)));
			}
			...
			this.startTimeLabelArray[num].text = dateTime4.ToString("H:mm");
			this.endTimeLabelArray[num].text = dateTime5.ToString("H:mm");
