...
public class GUISelectPanelBSUD : GUICollider
{
	...
	public void HideOutParts()
	{
		for (int i = 0; i < this.partObjs.Count; i++)
		{
			...
			float y = this.partObjs[i].gameObject.transform.position.y;
			if (y >= 600f || y <= -600f)
			{
	...
	protected void UpdateActive()
	{
		for (int i = 0; i < this.partObjs.Count; i++)
		{
			GameObject gameObject = this.partObjs[i].gameObject;
			Vector3 a = gameObject.transform.localPosition;
			a += base.gameObject.transform.localPosition;
			bool flag = false;
			if (this.partObjs[i].GetActCT() > 0 || this.partObjs[i].IsFadeDo())
			{
				flag = true;
			}
			if (!flag && (a.y > this.activeMargin + GUIMain.VerticalSpaceSize || a.y < -this.activeMargin - GUIMain.VerticalSpaceSize))
			{
				if (gameObject.activeSelf)
				{
					...
					{
						this.DoPanelAreaOverTopAction(this.partObjs[i]);
					}
					else if (a.y < -this.activeMargin - GUIMain.VerticalSpaceSize)
					{
						this.DoPanelAreaOverBottomAction(this.partObjs[i]);
					}
				}
			}
	...
	public void FadeOutAllListParts(Action<int> act = null, bool immediate = false)
	{
		...
		{
			this.actFadeAllCT++;
			this.partObjs[i].FadeOutEfc(new Action<int>(this.FadeAllCB), immediate);
		}
	}
	...
	public void FadeInAllListParts(Action<int> act = null)
	{
		...
		{
			this.actFadeAllCT++;
			this.partObjs[i].FadeInEfc(new Action<int>(this.FadeAllCB));
		}
	}
