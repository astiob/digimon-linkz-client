...
public sealed class CMD_FacilityShop : CMD
{
	...
	[SerializeField]
	private UILabel buildCostNumTitle;
	...
	private void InitFacilityShop(Action<int> closeEvent, float sizeX, float sizeY, float showTime)
	{
		...
		this.buildCostNumTitle.text = StringMaster.GetString("FacilityShopCostTitle");
	...
	private void UpdateCount()
	{
		...
		this.coinNum.text = StringFormat.Cluster(playerInfo.gamemoney);
	...
	{
		CMD_FacilityShop.<SetFacilityDetail>c__AnonStorey33B <SetFacilityDetail>c__AnonStorey33B = new CMD_FacilityShop.<SetFacilityDetail>c__AnonStorey33B();
		<SetFacilityDetail>c__AnonStorey33B.items = listUI.GetComponentsInChildren<FacilityShopItem>();
		if (<SetFacilityDetail>c__AnonStorey33B.items == null)
		{
			return;
		}
		int i;
		for (i = 0; i < <SetFacilityDetail>c__AnonStorey33B.items.Length; i++)
		{
			...
			<SetFacilityDetail>c__AnonStorey33B.items[i].SetDetail(facilityData[i], facilityCondition, isClearConditions, new Action<FacilityShopItem>(this.OnPushedBuyButton));
			BoxCollider component = <SetFacilityDetail>c__AnonStorey33B.items[i].GetComponent<BoxCollider>();
			if (null != component)
			{
			...
			{
				<SetFacilityDetail>c__AnonStorey33B.items[i].SetNewIcon(this.newFacilityItemList.Any((int id) => id == <SetFacilityDetail>c__AnonStorey33B.items[i].facilityID));
			}
			else
			{
				<SetFacilityDetail>c__AnonStorey33B.items[i].SetNewIcon(false);
			}
		}
	}
	...
	{
		CMD_FacilityShop.<SetNewIcon>c__AnonStorey33D <SetNewIcon>c__AnonStorey33D = new CMD_FacilityShop.<SetNewIcon>c__AnonStorey33D();
		<SetNewIcon>c__AnonStorey33D.items = listUI.GetComponentsInChildren<FacilityShopItem>(true);
		if (<SetNewIcon>c__AnonStorey33D.items == null)
		{
			return;
		}
		int i;
		for (i = 0; i < <SetNewIcon>c__AnonStorey33D.items.Length; i++)
		{
			bool newIcon = false;
			if (this.newFacilityItemList != null)
			{
				newIcon = this.newFacilityItemList.Any((int id) => id == <SetNewIcon>c__AnonStorey33D.items[i].facilityID);
			}
			<SetNewIcon>c__AnonStorey33D.items[i].SetNewIcon(newIcon);
		}
		this.newFacilityItemList = null;
	}
