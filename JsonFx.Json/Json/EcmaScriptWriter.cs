...
namespace JsonFx.Json
{
	public class EcmaScriptWriter : JsonWriter
	{
		...
		public static bool WriteNamespaceDeclaration(TextWriter writer, string ident, List<string> namespaces, bool isDebug)
		{
			...
			string[] array = ident.Split(EcmaScriptWriter.NamespaceDelims, StringSplitOptions.RemoveEmptyEntries);
			string text = array[0];
			bool flag = false;
			for (int i = 0; i < array.Length - 1; i++)
			{
				...
				if (!namespaces.Contains(text) && !EcmaScriptWriter.BrowserObjects.Contains(text))
				{
					namespaces.Add(text);
					if (i == 0)
					{
		...
		public static void WriteEcmaScriptDate(JsonWriter writer, DateTime value)
		{
			...
			{
				writer.TextWriter.Write("new Date({0:0000},{1},{2},{3},{4},{5},{6})", new object[]
				{
					value.Year,
					value.Month - 1,
					value.Day,
			...
			long num = (long)value.Subtract(EcmaScriptWriter.EcmaScriptEpoch).TotalMilliseconds;
