...
namespace UnityStandardAssets.ImageEffects
{
	...
	public class NoiseAndGrain : PostEffectsBase
	{
		...
		public override bool CheckResources()
		{
			...
			this.noiseMaterial = base.CheckShaderAndCreateMaterial(this.noiseShader, this.noiseMaterial, string.Empty);
			if (this.dx11Grain && this.supportDX11)
			{
				this.dx11NoiseMaterial = base.CheckShaderAndCreateMaterial(this.dx11NoiseShader, this.dx11NoiseMaterial, string.Empty);
			}
			if (!this.isSupported)
			{
		...
		private void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			if (!this.CheckResources() || null == this.noiseTexture)
			{
				...
				{
					global::Debug.LogWarning("Noise & Grain effect failing as noise texture is not assigned. please assign.", base.transform);
				}
				return;
			}
