...
namespace UniRx.Operators
{
	internal class CombineLatestObservable<T1, T2, T3, T4, T5, T6, TR> : OperatorObservableBase<TR>
	{
		...
		{
			return new CombineLatestObservable<T1, T2, T3, T4, T5, T6, TR>.CombineLatest(6, this, observer, cancel).Run();
		}
		...
		private class CombineLatest : NthCombineLatestObserverBase<TR>
		{
			...
			public CombineLatest(int length, CombineLatestObservable<T1, T2, T3, T4, T5, T6, TR> parent, IObserver<TR> observer, IDisposable cancel) : base(length, observer, cancel)
			{
				this.parent = parent;
			}
