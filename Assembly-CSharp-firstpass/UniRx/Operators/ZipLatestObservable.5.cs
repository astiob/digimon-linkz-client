...
namespace UniRx.Operators
{
	internal class ZipLatestObservable<T1, T2, T3, T4, T5, TR> : OperatorObservableBase<TR>
	{
		...
		{
			return new ZipLatestObservable<T1, T2, T3, T4, T5, TR>.ZipLatest(5, this, observer, cancel).Run();
		}
		...
		private class ZipLatest : NthZipLatestObserverBase<TR>
		{
			...
			public ZipLatest(int length, ZipLatestObservable<T1, T2, T3, T4, T5, TR> parent, IObserver<TR> observer, IDisposable cancel) : base(length, observer, cancel)
			{
				this.parent = parent;
			}
