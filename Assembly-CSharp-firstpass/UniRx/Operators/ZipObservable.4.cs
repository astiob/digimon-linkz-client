...
namespace UniRx.Operators
{
	internal class ZipObservable<T1, T2, T3, T4, TR> : OperatorObservableBase<TR>
	{
		...
		{
			return new ZipObservable<T1, T2, T3, T4, TR>.Zip(this, observer, cancel).Run();
		}
		...
		private class Zip : NthZipObserverBase<TR>
		{
			...
			public Zip(ZipObservable<T1, T2, T3, T4, TR> parent, IObserver<TR> observer, IDisposable cancel) : base(observer, cancel)
			{
				this.parent = parent;
			}
